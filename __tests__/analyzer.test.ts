...
`;
    ...
    const rules: CommentRules = { customInstructions: 'Focus on code quality' };
    ...
    expect(mockedAxios.post.mock.calls[0][2]?.headers).toHaveProperty('Authorization', 'Bearer test-api-key');
  });
...
`;
    ...
    const result = await analyzeDiff(diff, apiKey, rules, 'anthropic', 'claude-3-7-sonnet-20250219');
    ...
    expect(mockedAxios.post.mock.calls[0][1]).toHaveProperty('model', 'claude-3-7-sonnet-20250219');
    expect(mockedAxios.post.mock.calls[0][1]).toHaveProperty('messages');
    expect(mockedAxios.post.mock.calls[0][2]?.headers).toHaveProperty('x-api-key', 'test-api-key');
    expect(mockedAxios.post.mock.calls[0][2]?.headers).toHaveProperty('anthropic-version', '2023-06-01');
  });
  ...
  test('analyzeDiff should handle invalid provider', async () => {
    ...
    await expect(analyzeDiff(diff, apiKey, rules, 'invalid-provider' as LLMProvider))
